...
export function makeBookingRow(
  ...
  const court = mockCourts.find((c: { id: number }) => c.id === booking.court_id);
  const courtType = court
    ? mockCourtTypes.find((ct: { id: number }) => ct.id === court.court_type_id)
    : null;
  const branch = court
    ? mockBranches.find((b: { id: number }) => b.id === court.branch_id)
    : null;
  const customer = mockCustomers.find((c: { id: number }) => c.id === booking.customer_id);
  const employee = booking.employee_id
    ? mockEmployees.find((e: { id: number }) => e.id === booking.employee_id)
    : null;
...
export function makeBookingDetailView(
  ...
  const courtTypeData = mockCourtTypes.find(
    (ct: { id: number }) => ct.id === court.court_type_id
  )!;
  const branch = mockBranches.find((b: { id: number }) => b.id === court.branch_id)!;
  const customer = mockCustomers.find((c: { id: number }) => c.id === booking.customer_id)!;
  const employee = booking.employee_id
    ? mockEmployees.find((e: { id: number }) => e.id === booking.employee_id)
    : undefined;
  ...
  const serviceItemDetails: ServiceBookingItemDetail[] = serviceItems.map(
    (item) => {
      const branchService = mockBranchServices.find(
        (bs: { id: number }) => bs.id === item.branch_service_id
      )!;
      const service = mockServices.find(
        (s: { id: number }) => s.id === branchService.service_id
      )!;
