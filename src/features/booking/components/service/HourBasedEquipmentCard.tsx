...
export function HourBasedEquipmentCard({
  ...
  const updateHourEntry = (entryId: string, hours: number) => {
    const newEntries = hourEntries.map((entry: { id: string; hours: number }) =>
  ...
  const removeHourEntry = (entryId: string) => {
    const newEntries = hourEntries.filter((entry: { id: string }) => entry.id !== entryId);
    onUpdate(item.id, newEntries.length, newEntries);
  };

  const totalHours = hourEntries.reduce((sum: number, entry: { hours: number }) => sum + entry.hours, 0);
          ...
          <div className="flex-1 space-y-3">
            ...
            {hourEntries.length > 0 && (
              <div className="space-y-2">
                {hourEntries.map((entry: { id: string; hours: number }, index: number) => (
                  <div
                    key={entry.id}
